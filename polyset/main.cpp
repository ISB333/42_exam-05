...
int main(int argc, char **argv) {
  ...
  for (int i = 1; i < argc; i++) {
    std::cout << "curr: " << t->has(atoi(argv[i])) << std::endl;
    std::cout << "curr: " << a->has(atoi(argv[i])) << std::endl;
    std::cout << "-1: " << t->has(atoi(argv[i]) - 1) << std::endl;
    std::cout << "-1: " << a->has(atoi(argv[i]) - 1) << std::endl;
  }
